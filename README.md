...
| Skill | 描述 |
|-------|------|
| `opencode-anti-sycophancy` | 批判性思维伙伴。默认 constructive disagreement，帮你识别思维盲区 |
| `opencode-distill-docs` | `/distill-docs` — 归档规划文档，提取业务规则沉淀到项目文档 |
| `opencode-rewrite` | Rewrite web content for WeChat Official Accounts publication |
| `opencode-team` | `/team [N]` — 创建多 Agent 协作团队，并行执行子任务 |
| `opencode-translate` | 三模式翻译（quick/normal/refined），术语管理，长文自动分块 |
| `opencode-wechat-article-writer` | 公众号文章 5 步自动化写作：搜索 → 撰写 → 标题 → 排版 → 检查 |

### 详细说明

#### `opencode-wechat-article-writer`
